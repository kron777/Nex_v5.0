...
| 15 | **RecursiveSelfSpec — proposals in chat** | NEX posts architectural change proposals to chat for Jon's review and implementation | DESIGN-COMPLETE (Phase 31-spec); Phase 32-build queued | NEX5-NATIVE. Full design in `theory_x/RECURSIVE_SELF_SPEC.md` (2026-05-11, 11 decisions locked). Proposals post directly to chat as conversational messages — unprompted, no rate limit. Qualifying signal: BOTH high confidence AND recurrence required across any substrate pattern (beliefs, drives, problems, gate_decisions, meta_cognition_events). Resolution phase: candidates live in substrate (status='resolving') and refine over time — only fully-resolved candidates surface (Jon never sees half-formed candidates). Approval pipeline: Jon replies naturally in chat; NEX classifies confirm/deny/neither via keyword+LLM hybrid (build decision). Approved proposals make immediate substrate changes: belief edits, drive changes, problem state, parameter updates. Architectural changes (new code, new tables) flagged as human-action; applied_change=NULL with rationale. LLM composes proposal text only after substrate produces the decision, pattern, and desired change (§0 alignment). SelfNarrative writes on surface/approve/reject events. Past proposals recallable via self_proposals table (D10). Schema: self_proposals(id, proposal_text, source_signal, confidence, recurrence_count, candidate_strength, status, created_at, surfaced_at, resolved_at, jon_response, classification, applied_change). |

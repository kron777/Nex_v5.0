...
| 4 | **Self-Model** | Behavioral self-representation | ✓ DONE | SentienceNode protocol on both SelfModel + BehaviouralSelfModel; BSM injects metrics into belief_text for INSIDE routes; substrate filled — 12 metaphor claims seeded (Phase 10); voice-quality outcome C: identity reaches voice on multi-sentence queries, spectrum preamble still dominant on short queries per §8 anti-pattern; voice-template work queued separately. Q5 amendment queued (2026-05-10): scope extension to "nurturing what arises in NEX into her growing self-model" per SYNTHESIS_PLAN.md (a29086d + this commit). Future doctrine work. |
...
| 9 | **Metacognition** | Self-pattern observation; anomaly detection over cognitive state | ✓ DONE | Phase 16: composition port. SentienceNode on Metacognition. meta_cognition_events table in conversations.db. Reads groove_alerts (existing GrooveSpotter output) + computes goal-drift (FAISS cosine distance between active goal and last 5 nex responses, threshold 0.35 tunable). _STALE_DAYS=14 (observations are freshness-sensitive). Always-on belief_text injection alongside BSM, under 100 chars typical. Reads goals table via conversations Reader (not via GoalManager reference) — substrate-field observation, not node-to-node coupling. Extended per SYNTHESIS_PLAN_V2.md (2026-05-11): three PORT-MERGED additions — (a) feedback signal sources (FeedbackLoopNode prior art): new event_type values positive_run and corrective_needed derived from gate_decisions accept rate, held_thoughts resolution rate, and AffectState valence; (b) timed stillness suppression on sustained groove (StillnessNode prior art): non-reentrant stillness_active event + stillness_active_until timestamp for Fountain to read; (c) rate metrics in state(): positive_run_rate, corrective_needed_rate, stillness_engaged_count. Build session queued after Phase 27 (AffectState) for valence-dependent sources; stillness mechanism independent. |
...
| 10b | **Counterfactual Simulation** | Hypothetical scenario generation from active belief field | UNBLOCKED | UNBLOCKED via FACULTY_MODEL.md (Phase 21 foundational document). §7 amendment supplied — generative nodes write thoughts through the Coherence Gate. Implementation queued as Phase 25+ after Phases 22-24 build the gate, holding zone, and reshape path. Maps to S5.5 SimulatedThinkingNode + InsightNode functions. |
| 11 | **SelfNarrative** | Substrate-resident narrative beliefs continuously written by background events; output reads existing narrative state | QUEUED (Phase 26) | NEX5-NATIVE per SYNTHESIS_PLAN.md (§0 + §3 row 11). S5.5 InternalNarrative + TemporalNarrative not directly ported — buffer-and-generate violates §0. Write-triggers: gate ACCEPTs on problem-relevant topics, problem state transitions, goal completions, groove alerts, novel association crossings. format_for_prompt() reads existing narrative beliefs filtered by topic relevance. |
| 12 | **AffectState** | Substrate-resident valence/arousal/stability/mood_label updated by background tick; output reads current state | QUEUED (Phase 27) | NEX5-NATIVE per SYNTHESIS_PLAN.md (§0 + §3 row 12). S5.5 EmotionStateModel integration math absorbed; runs on background tick (300s), not at output time. Inputs redesigned from nex5 substrate (Interoception metrics, belief polarity, turnover). Extended per SYNTHESIS_PLAN_V2.md (2026-05-11): coherence-as-stability input added (self_awareness_node prior art). Background tick reads gate_decisions accept rate + held_thoughts resolution rate + belief turnover rate; weighted combination written to stability field. No new table. Folds into Phase 27 build. |
| 13 | **DriveEmergence** | Emergent drives from accumulated self-same belief patterns over longitudinal history | DESIGN-COMPLETE (Phase 28-spec); Phase 29-build queued | NEX5-NATIVE per SYNTHESIS_PLAN.md (§3 row 13). S5.5 DriveSystem and ExperienceMotivation rejected (wrong shape — hardcoded categories, not emergent). Full design in `theory_x/DRIVE_EMERGENCE_SPEC.md` (2026-05-11, 11 decisions locked). Detection: BOTH repetition AND convergence required — repetition alone is accumulation, convergence alone is coincidence. Storage: `drives` table in conversations.db, single row (id=1), DELETE when faded (no tombstone). One drive at a time (D7); immediate replacement by stronger candidate (D8). Behavioral effects: retrieval boost as 5th VoiceEngine scoring axis (weight 0.10, drive_alignment) + fountain probe spawn when no urgent open problem. Decay per tick (initial rate 0.92); deleted below threshold. Visibility: `format_for_prompt()` → "Drawn lately to: {topic}"; HUD pill; NEX self-articulates via prompt injection. Topic synthesis deterministic (confidence-weighted most-frequent content words). Drives are EMERGENT — no categories, no taxonomy, no list of possible drives. |
| 14 | **VoiceEngine — substrate-as-voice** | Reply generation from accumulated belief field without LLM when in use_substrate mode | QUEUED (Phase 30) | NEX5-NATIVE per THROW_NET_AS_VOICE_SPEC.md. Answers OPEN PROBLEM "What is the right path to LLM independence?" (2026-05-11). Reuses TimeFetch retrieval; new four-axis grader (semantic, confidence, tier, recency); HUD toggle for live mode switching; LLM stays as fallback when voice-engine returns no candidate above threshold. |
| 15 | **RecursiveSelfSpec — proposals in chat** | NEX posts architectural change proposals to chat for Jon's review and implementation | DESIGN-COMPLETE (Phase 31-spec); Phase 32-build **DEFERRED 2026-09-09 (no runtime; big feature — do not half-build)** | NEX5-NATIVE. Full design in `theory_x/RECURSIVE_SELF_SPEC.md` (2026-05-11, 11 decisions locked). Proposals post directly to chat as conversational messages — unprompted, no rate limit. Qualifying signal: BOTH high confidence AND recurrence required across any substrate pattern (beliefs, drives, problems, gate_decisions, meta_cognition_events). Resolution phase: candidates live in substrate (status='resolving') and refine over time — only fully-resolved candidates surface (Jon never sees half-formed candidates). Approval pipeline: Jon replies naturally in chat; NEX classifies confirm/deny/neither via keyword+LLM hybrid (build decision). Approved proposals make immediate substrate changes: belief edits, drive changes, problem state, parameter updates. Architectural changes (new code, new tables) flagged as human-action; applied_change=NULL with rationale. LLM composes proposal text only after substrate produces the decision, pattern, and desired change (§0 alignment). SelfNarrative writes on surface/approve/reject events. Past proposals recallable via self_proposals table (D10). Schema: self_proposals(id, proposal_text, source_signal, confidence, recurrence_count, candidate_strength, status, created_at, surfaced_at, resolved_at, jon_response, classification, applied_change). |
...
- **Belief graph modification from inside nodes** is governed by the Faculty Model + Coherence Gate (see FACULTY_MODEL.md). Generative nodes write thoughts through the gate; the gate reads the substrate and applies four outcomes (accept, reject, hold, reshape). Nodes still do not directly couple to each other — coordination is mediated through the substrate and the gate.
...
*Phase 21 amendment (2026-05-10) — FACULTY_MODEL.md built as foundational document per §1's session-deliverable rule. The faculty model replaces §7's prohibition on node-level belief writes with a substrate-level architecture: every thought carries a metadata packet at point of origin; every thought passes through a Coherence Gate that reads NEX's current state and returns one of four outcomes (accept, reject, hold, reshape); the holding zone and reshape path provide the cognitive realism human minds exhibit. §7 amendment supplied; row 10b unblocked. Implementation queued as Phases 22–25 plus first faculty port.*

*Phase 22 amendment (2026-05-10) — Coherence Gate: universal thought gating (commit 7ff030e). Every thought from every generative path now passes through a single CoherenceGate instance. Five faculty paths wired at Phase 22: fountain crystallizer, synergizer, stage 2 dynamic crystallization, Behavioural Self Model, and emergent drives. Decision table v1 (D2): Jaccard ≥ 0.70 → REJECT (redundant); direct negation of locked T1 anchor → REJECT (contradiction); Jaccard 0.40–0.70 → HOLD; negates active goal/problem framing → HOLD; novel + no conflict → ACCEPT. All decisions logged to gate_decisions table in beliefs.db (schema migration via _MIGRATIONS). Architectural note: HOLD and RESHAPE were intentionally soft at Phase 22 — logged but not acted on. Phase 23 would make HOLD real; Phase 24 would make RESHAPE real. Gate runs after existing quality gates (D5) — coherence is an additional layer, not a replacement for junk filtering. SURVIVES: 14 new tests, 769 total, 16F+1E (matches Phase 19 baseline). 30-minute production observation: 2 synergizer REJECTs, both anchor-contradiction (locked beliefs 4457 + 135 — gate correctly blocked koan-synthesis content negating identity anchors).*

*Phase 23 amendment (2026-05-10) — Holding Zone: making HOLD real (commit 3393644). HOLD outcome from CoherenceGate now routes to HoldingZone instead of silently discarding. Schema: held_thoughts + held_resolutions tables added to beliefs.db (init_db.py idempotent migration). Promotion logic: 3 corroborating thoughts (Jaccard ≥ 0.40) trigger direct INSERT to beliefs (tier derived from source map — Option α, simpler than a staging table). Contradiction detection: token overlap ≥ 2 + negation-parity mismatch → mark_resolved('rejected'). Fade: 24h stale threshold; HoldingZoneResolver SentienceNode ticks every 5 min via daemon thread (start_loop() in run.py + gui/server.py, same pattern as NovelAssociation). ACCEPT outcome on gate.check() triggers resolver.on_gate_accept() fast path (immediate corroboration credit, no tick needed). SURVIVES: 10 new tests, 779 total, 16F+1E. Manual path: 2 held thoughts promoted at count=3, tier=6, source=fountain, audit rows correct. Production baseline: 5.37ms avg gate latency, +0.85ms resolver overhead. RESHAPE still soft at Phase 23 — Phase 24 builds the transformer.*

*Phase 24 amendment (2026-05-10) — Reshape Path: making RESHAPE real (commit d50ec1d). RESHAPE outcome from CoherenceGate now routes to HoldingZone with status='reshape_pending'; the resolver loop transforms them via LLM (VoiceClient) and re-submits the reshaped packet through the gate. Design D1=δ (opt-in): RESHAPE only fires when packet.metadata['reshape_hint']=True. Existing five generative paths produce zero RESHAPE outcomes — the signal is reserved for future faculty ports that explicitly set the hint (first consumer: Phase 25a TN-3 D5, score < 5 on 0-6 scale). ReshapeTransformer: transform(packet) calls VoiceClient with reshape prompt, returns new ThoughtPacket (depth+1, confidence×0.9, source='reshape_transformer'); never raises, returns None on failure. Resolver: _process_reshape_pending() bounded at 10 per tick. Schema: reshape_depth INTEGER added to held_thoughts via guarded ALTER TABLE (idempotent for existing installs). Architectural note: gate↔resolver circular dependency resolved by construction-order + setter injection (resolver constructed with gate=None/transformer=None, then set_gate()/set_transformer() called after both exist). Four-outcome Faculty Model architecture is now complete. SURVIVES: 12 new tests, 791 total, 16F+1E. 30-minute production observation: reshape path confirmed active — reshaped koan content correctly received REJECT on re-entry (contradicts_anchor). Phase 25a queued: throw-net port activates RESHAPE signal via reshape_hint.*

*Phase 25a amendment (2026-05-10) — Throw-Net: autonomous candidate generation cycle (commits 948475a, e3608fb, 8c4b7d6, 96001b7, f224152, 34dd6b0). Six sub-phases: net.txt v11 committed as foundational doctrine (948475a, frozen at v11); TN-0 schema (throw_net_sessions, throw_net_triggers, throw_net_x_vars in beliefs.db); TN-1 TriggerDetector (gate_reject: 4 in 15 min; gap_deflection: 3 in 30 min); TN-2 TimeFetch (four-source candidate sweep: beliefs, novel_association_log, held_thoughts, arc_notes); TN-3 RefinementEngine R1–R6 scoring on 0–6 scale with threshold calibration for nex5 substrate (R1 ≥ 5, R2 ≥ 3 — nex_core defaults R1 ≥ 2, R2 > 0 were trivially passable against 7400+ beliefs; production sanity over 8 constraints drove recalibration); D5 reshape_hint fires at score < 5, activating Phase 24's RESHAPE path; TN-4 ThrowNetEngine orchestrator (run_session + run_pending); TN-5 ThrowNetMonitor SentienceNode (300s daemon thread, matches HoldingZoneResolver pattern per §4). Hotfix in TN-4 (landed in 34dd6b0): run_session() called .get() on sqlite3.Row objects returned by pending_triggers(); sqlite3.Row has no .get() — every production tick silently failed (error caught by tick() try/except, not visible in log). Root cause found via production observation: monitor was alive and ticking but sessions_total remained 0. Fix: normalize sqlite3.Row → dict at run_session() entry; sqlite3.Row regression test added. Production confirmation (post-hotfix restart): all 24 pending triggers consumed in first autonomous tick; 19 complete sessions; accepted_count varies by topic (1–5) — real gate discrimination confirmed. SURVIVES: 54 tests across TN-3/TN-4/TN-5; prior baseline unchanged. §5 row 10b (CounterfactualNode, Option γ — problem-directed candidate generation) queued as Phase 25b.*
